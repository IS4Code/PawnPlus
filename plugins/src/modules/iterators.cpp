	...
	return value_read(source.get(), [&](const dyn_object &val)
	{
		...
		try{
			return key_read(source.get(), [&](const dyn_object &key)
			{
				...
				return expr->execute_bool(amx, args, *env);
			});
...
bool project_iterator::extract_dyn(const std::type_info &type, void *value) const
{
	if(auto lock = amx.lock())
	{
		if(type == typeid(std::shared_ptr<const dyn_object>))
		{
			...
			{
				*reinterpret_cast<std::shared_ptr<const dyn_object>*>(value) = std::make_shared<dyn_object>(expr->execute(*lock, {std::ref(obj)}, *env));
				return true;
			});
		}else if(type == typeid(std::shared_ptr<const std::pair<const dyn_object, dyn_object>>))
		{
			return value_read(source.get(), [&](const dyn_object &val)
			{
				return key_read(source.get(), [&](const dyn_object &key)
				{
					*reinterpret_cast<std::shared_ptr<const std::pair<const dyn_object, dyn_object>>*>(value) = std::make_shared<std::pair<const dyn_object, dyn_object>>(key, expr->execute(*lock, {std::ref(val), std::ref(key)}, *env));
					return true;
				});
			});
		}
