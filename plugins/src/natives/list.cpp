...
namespace Natives
{
	...
	// native List:list_var_args(const format[], bool:byref=false, level=0);
	static cell AMX_NATIVE_CALL list_var_args(AMX *amx, cell *params)
	{
		char *format;
		...
		{
			logerror(amx, "[PP] list_var_args: not enough arguments");
			return 0;
		}
		...
		if(format != nullptr) for(size_t i = 0; i <= numargs; i++)
		{
			...
			switch(format[i])
			{
				...
				case '+':
				{
					logerror(amx, "[PP] list_var_args: + must be the last specifier");
...
static AMX_NATIVE_INFO native_list[] =
{
	...
	AMX_DECLARE_NATIVE(list_var_args),
